...
def checkInput(input):
    """
    validate input is valid. If not, print "Invalid Input!" and exit with code 1
    """
    try:
        assert (len(input) == 4)
...
def reorderEigen(eigens, n):
    """
    sorts the eigen values returned from jacobi method in decreasing order
    """
...
def eigengap(mat, n):
    """
    returns k according to the eigengap heuristic
    """
    max_gap = 0
    k = 0
    for i in range(n//2):
...
def kGreatestCols(eigens, n, k):
    """
    returns the k eigen vectors that correspond with the k largest eigen values
    """
...
def initialPoints(data_points, k, n, d):
    ...
    # initialize a 2 columns, N rows table
    distance_table = np.full((2, n), math.inf)
    for i in range(k-1):
        # first column of distance table is the distance of each vector from the current centroid
        distance_table[0] = np.minimum(distance_table[0], np.sum(
            np.power(data_points - initial_centroids[i], 2), axis=1))
        sumOdds = np.sum(distance_table[0], axis=0)
        # second column of distance table is the probability for each vector to be the next centroid
        np.true_divide(distance_table[0], sumOdds, out=distance_table[1])
...
def main(k, goal, filename):
    ...
    if goal == goals.WAM.value:
        ...
        T = renormalize(U, n, k)
        (initial_indices, initial_centroids) = initialPoints(
            T, k, len(T), len(T[0]))
        if (k == 1):
        ...
        final_centroids = spkmodule.spk(
            c_points, c_centroids, len(T), k, len(T[0]))
